...
app = Flask(__name__)
config = {"img_size": 640, "stride": model.model.stride.max().item(),
          "prediction": ["x1", "y1", "x2", "y2", "conf", "class", "pitch", "yaw", "roll"]}


@app.route('/predict', methods=['POST'])
def predict():
    img, old_shape = preprocess_img(extract_img(request).read(), config["img_size"], config["stride"])
    ...
    out[:, :4] = scale_coords(img.shape[2:], out[:, :4], old_shape[:2])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--img-size', type=int, default=640, help='Image resize size')
    parser.add_argument('-s', '--stride', type=int, default=model.model.stride.max().item(), help='Image resize stride')
